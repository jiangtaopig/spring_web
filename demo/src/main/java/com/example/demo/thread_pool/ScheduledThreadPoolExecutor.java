...
public class ScheduledThreadPoolExecutor
    ...
    public ScheduledFuture<?> schedule(Runnable command,
        ...
        RunnableScheduledFuture<?> t = decorateTask(command,
                new ScheduledThreadPoolExecutor.ScheduledFutureTask<Void>(command, null,
    ...
    public <V> ScheduledFuture<V> schedule(Callable<V> callable,
        ...
        RunnableScheduledFuture<V> t = decorateTask(callable,
                new ScheduledThreadPoolExecutor.ScheduledFutureTask<V>(callable,
                        triggerTime(delay, unit)));
        delayedExecute(t);
        return t;
    }
    ...
    public ScheduledFuture<?> scheduleAtFixedRate(Runnable command,
        ...
        ScheduledThreadPoolExecutor.ScheduledFutureTask<Void> sft =
                new ScheduledThreadPoolExecutor.ScheduledFutureTask<Void>(command,
                        null,
    ...
    static class DelayedWorkQueue extends AbstractQueue<Runnable>
        ...
        public RunnableScheduledFuture<?> poll(long timeout, TimeUnit unit)
            ...
            try {
                for (;;) {
                    RunnableScheduledFuture<?> first = queue[0];
                    System.out.println("RunnableScheduledFuture  >>> first = " + first);
                    if (first == null) {
                        if (nanos <= 0)
                            return null;
                        else
                        ...
                        long delay = first.getDelay(NANOSECONDS);
                        System.out.println("RunnableScheduledFuture  >>> delay = " + delay + " , nanos = " + nanos);
                        if (delay <= 0)
                            return finishPoll(first);
                        if (nanos <= 0)
                            return null;
